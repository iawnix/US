...
def MyColorNorm():
    ...
    colors_land = plt.cm.jet(np.linspace(0.001, 1, 256))
    ...
    #divnorm = colors.TwoSlopeNorm(vmin=0.0, vcenter=9.75, vmax=19.5)
...
def draw2(xyz,mycn):
    ...
    #Pcolor= plt.pcolor(X,Y, Z,shading='auto',cmap=mycn[0], norm=mycn[1])      
    import matplotlib.colors as colors
    Pcolor= plt.pcolor(X,Y, Z,shading='auto',cmap = "jet",norm = colors.TwoSlopeNorm(vmin=0.0, vcenter=11.5, vmax=23) )
    
    ax.invert_xaxis()

    ax.invert_yaxis()
    cb = ax.figure.colorbar(Pcolor,ax=ax,location="right")
    
    #ax.set_title("Free Energy Profile (kcal/mol)",pad = 20,x=0,y=0.5)
    cb.set_label("Free Energy Profile (kcal/mol)",labelpad = 20)
...
def draw3(xyz,mycn):
    ...
    #plt.xticks([-1,0,1,2,3],rotation = 135)
    #ax.set_zticks([0, 5 , 10, 15, 20, 25])
    ax.set_xticks([-0.5,0.5,1.5,2.5])
    ax.set_yticks([-2.0,-1,0,1,2])
